        ...
        for _, row in games_df.iterrows():
            ...
            if game_id:
                try:
                    ...
                    if our_row.empty:
                        raise ValueError(f"No row found for teamId {team_id} in linescore for game {game_id}")
                    team_pts = int(our_row.iloc[0].get("PTS") or 0)
                    opp_row = game_scores[game_scores["TEAM_ID"] != team_id]
                    if opp_row.empty:
                        raise ValueError(f"No opponent row found in linescore for game {game_id}")
...
sb = ScoreboardV2(game_date="2026-02-02")
GAME_ID = "0022500715"
team_id = 1610612755
linescore = sb.get_data_frames()[1]

game_scores = linescore[linescore["GAME_ID"] == GAME_ID]
our_row = game_scores[game_scores["TEAM_ID"] == team_id]
print(our_row)
